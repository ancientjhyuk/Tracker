...
document.getElementById('reservationForm').addEventListener('submit', function(event) {
    ...
    const reservationData = { date, timestamp, transactionType, ticketNumber, routeType, airportCode, tagging, icName, pnr, description, urgency: urgencyShort };
...
function addReservationToTable(data) {
    ...
    deleteButton.style.backgroundColor = '#d32f2f'; // Red color for delete button
    ...
    rows.forEach(row => {
        ...
        cells.forEach((cell, index) => {
            const key = Object.keys({
                ...
                description: '',
                urgency: ''
            })[index]; // Update to reflect correct keys
...
function clearAllReservations() {
    ...
    localStorage.removeItem('reservations'); // Clear from localStorage
}
